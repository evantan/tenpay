...
Payment.prototype._validate = function(xml, type, callback) {
	...
	util.parseXML(xml, function (err, json) {
		...
		if (json.return_code != 'SUCCESS') return callback(null, json || '消息格式错误: return_msg');
		if (json.result_code != 'SUCCESS') return callback(null, json || 'err_code格式错误', json.err_code_des);
